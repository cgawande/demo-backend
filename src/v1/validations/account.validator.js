...
const emailOrMobile = Joi.alternatives()
  .try(
    Joi.string()
      .trim()
      .label("Phone Number")
    ...
    Joi.string().trim().label("Email").required()
  )
  .messages({
...
const updateData = {
  firstName: Joi.string().trim().min(3).max(24).label("First Name").required(),
  lastName: Joi.string()
    .trim()
    .min(3)
    .max(24)
    ...
    .allow(null)
    .allow(""),
  profilePicture: Joi.string().trim().optional().empty().allow(""),
...
const editProfileDetailsSchema = {
  body: Joi.object().keys({
    ...updateData,
    city: Joi.string().trim().min(2).max(30).optional().empty().allow(""),
    state: Joi.string().trim().min(2).max(30).optional().empty().allow(""),
    country: Joi.string().trim().min(2).max(20).optional().empty().allow(""),
    address: Joi.string().trim().min(2).max(60).optional().empty().allow(""),
    dateOfBirth: Joi.string().trim().optional().empty().allow(""),
    gender: Joi.string()
...
const userAccountLoginSchema = {
  body: Joi.object({
    ...
    firebaseToken: Joi.string().trim().optional().empty().allow(""),
...
const userAccountSignUpSchema = {
  body: Joi.object({
    ...
    //   .allow(null)
    //   .allow(''),
    fullName: Joi.string().trim().min(4).max(100).label("full Name").required(),
    ...
    email: Joi.string()
      ...
      .max(100)
      .required(),
    permissions: Joi.array().optional().items(Joi.array()).default([]),
  }),
};
