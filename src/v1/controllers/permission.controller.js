...
module.exports.getPermissionList = async (req, res, next) => {
    try {
      const result = await PermissionRepository.getPermissionList(req);
      if (result) {
  ...
  module.exports.addPermission = async (req, res, next) => {
    try {
      const result = await PermissionRepository.addPermission(req);
      if (result) {
        res.status(utility.httpStatus('OK')).json({
          success: true,
          data: result,
